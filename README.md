...
- where $p^t_{i,r}$ is the price set by agent $i$ at period $t$ of run $r$ of the experiment, $p^{t-1}\_{-i,r}$ is the price set by $i$'s competitor at period $t$ of run $r$, $\alpha\_{i,r}$ is a firm-run fixed effect.
